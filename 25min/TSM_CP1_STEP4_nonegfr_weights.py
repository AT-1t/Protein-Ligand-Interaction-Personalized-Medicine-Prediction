...
def make_bins(input_series, n_bins=10):
    ...
    unique_count =input_series.nunique()
    bin_count = int(max(2, min(n_bins, unique_count)))
    if bin_count < 2:
        return None
    try:
...
######## newly learned code 
            ...
            if any(weights_wrong):
                raise ValueError(f"Model weights turned nan and inifnite at epoch{epoch}, batch start{start}")
